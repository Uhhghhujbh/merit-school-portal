...
import {
   ...
   ChevronRight, Landmark, Send, BrainCircuit, Play
} from 'lucide-react';
...
const StudentDashboard = () => {
   ...
   // Subscription State (CBT/Quiz)
   const [showPaymentModal, setShowPaymentModal] = useState(null); // 'cbt' or 'quiz' or null
   const [subscription, setSubscription] = useState({ cbt: { active: false }, quiz: { locked: false } });
   ...
   const loadDashboardData = async () => {
      try {
         if (user?.id) {
            ...
            setAnnouncements(msgData?.announcements || []);
            setFees(feeData || { fee_jamb: 15000, fee_alevel: 27500, fee_olevel: 10000 });
            setResults(resultsData || []);

            // Check subscription status
            try {
               const subStatus = await api.get('/payments/subscription-status', token);
               if (subStatus) {
                  setSubscription(subStatus);
               }
            } catch (subErr) {
               console.log('Subscription check skipped:', subErr.message);
            }
         }
      } catch (err) {
   ...
   return (
      ...
      `}>
            ...
            <nav className="flex-1 p-4 space-y-2 overflow-y-auto custom-scrollbar">
               ...
               <div className="pt-2">
                  <p className="px-4 text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">Assessments & Social</p>
                  <SidebarItem icon={<BrainCircuit size={20} />} label="CBT Practice" onClick={() => navigate('/student/cbt')} />
                  <SidebarItem icon={<Play size={20} />} label="Quiz Hub" onClick={() => navigate('/student/quiz')} />
                  <SidebarItem icon={<Send size={20} />} label="Student Chat" onClick={() => navigate('/student/chat')} />
               </div>
                  ...
                  <div className="lg:col-span-2 space-y-8">
                     ...
                     <div className="bg-white rounded-2xl shadow-soft border border-slate-200 overflow-hidden">
                        ...
                        <div className="divide-y divide-slate-100 max-h-64 overflow-y-auto custom-scrollbar">
                           {!Array.isArray(announcements) || announcements.length === 0 ? <p className="p-6 text-center text-slate-400">No news yet.</p> :
                           ...
                           {paymentMethod === 'transfer' && (
                              <div className="space-y-4 animate-fadeIn">
                                 <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 text-sm">
                                    ...
                                    <p>Account Name: <span className="font-mono font-bold">Alaaya Opeyemi</span></p>
                                 </div>
                                 <div>
         ...
         {showPaymentModal && (
            <FlutterwavePayment
               ...
               onSuccess={() => {
                  ...
                  loadDashboardData(); // Refresh subscription status
               }}
